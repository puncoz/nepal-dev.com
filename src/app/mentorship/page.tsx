import ProgressTracker from "@/components/mentorship/progress-tracker"
import Badge from "@/components/ui/badge"
import Breadcrumb from "@/components/ui/breadcrumb"
import Card from "@/components/ui/card"
import { AlertCircle, Calendar, Clock, MessageSquare, Star, TrendingUp, UserCheck, Users } from "lucide-react"
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="mb-8">
          <Breadcrumb
            items={[{ label: "Mentorship" }]}
            className="mb-6"
          />
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Mentorship Hub</h1>
          <p className="text-gray-600">Connect with experienced developers and grow your career</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {quickActions.map((action, index) => (
              <Link key={index} href={action.href}>
                <div
                  className={`${action.color} text-white p-4 rounded-lg transition-all duration-200 hover:scale-105 hover:shadow-lg cursor-pointer group`}>
              ...
              {pendingActions.map((action) => (
                <div key={action.id}
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <button
                      className="ml-4 px-3 py-1 bg-blue-500 text-white text-sm rounded-lg hover:bg-blue-600 transition-colors">
                      Action
                    </button>
                  </div>
                </div>
