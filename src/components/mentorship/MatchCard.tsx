...
import { Star, MapPin, Clock, MessageSquare, Calendar, Bookmark, Eye, TrendingUp } from 'lucide-react';
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="text-right">
            <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold ${getMatchScoreColor(match.matchScore)}`}>
              <Star className="w-4 h-4 mr-1" />
              {match.matchScore}% Match
            </div>
            <div className="flex items-center mt-2">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className={`w-4 h-4 ${i < Math.floor(match.rating) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`} />
              ))}
